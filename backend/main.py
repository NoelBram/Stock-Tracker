...
# STOCK_DF = get_stock_df('my_stock_quotes', STOCK_NAME, '2021-12-23', TODAY)
...
nfeatures = 7
print('STOCK_DF.shape = {df}, df_train.shape = {df_train}, df_test.shape = {df_test}'.format(df = STOCK_DF.shape, df_train = df_train.shape, df_test = df_test.shape))
...
class Model:
    def __init__(
        ...
        self.X = Input(shape=(size,))
        self.Y = Dense(output_size, activation='softmax')(self.X)
        # self.X = tf.compat.v1.placeholder(tf.float32, (None, None, size))
        # self.Y = tf.compat.v1.placeholder(tf.float32, (None, output_size))

        drop = layers.RNN(stacked_lstm, return_sequences=True, return_state=True)
        # drop = tf.nn.RNNCellDropoutWrapper(
        #     stacked_lstm, output_keep_prob = forget_bias
        # )
        self.hidden_layer = Dense(num_layers * 2 * size_layer, activation='relu')(self.X)
        # self.hidden_layer = tf.compat.v1.placeholder(
        #     tf.float32, (None, num_layers * 2 * size_layer)
        # )

        self.outputs, self.last_state = drop([self.X, self.hidden_layer])
        # self.outputs, self.last_state = tf.nn.dynamic_rnn(
        #     drop, self.X, initial_state = self.hidden_layer, dtype = tf.float32
        # )
        self.logits = layers.Dense(self.outputs, output_size)
        self.cost = tf.compat.v1.reduce_mean(tf.square(self.Y - self.logits))
        self.optimizer = keras.optimizers.Adam(learning_rate).minimize(
            self.cost
        )
...
def forecast():
    ...
    date_ori = pd.to_datetime(STOCK_DF.iloc[:, 0]).tolist()
